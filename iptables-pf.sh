...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://spider-1300246764.cos.ap-beijing.myqcloud.com/iptables/iptables-pf.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	wget -N --no-check-certificate "https://spider-1300246764.cos.ap-beijing.myqcloud.com/iptables/iptables-pf.sh" && chmod +x iptables-pf.sh
